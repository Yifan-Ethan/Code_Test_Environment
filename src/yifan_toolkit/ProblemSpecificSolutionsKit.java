...
public class ProblemSpecificSolutionsKit {
		...
		public static BigInteger recursetrianglesearch(int index, int r, List<List<BigInteger>> listofelements, List<BigInteger> combi, int pn,BigInteger totalcount, BigInteger xmod, BigInteger ymod){
			...
			if(r==1){	
				for(int i=index;i<pn-r+1;i++){			//Iterate through each element within current loop
					...
					List<List<BigInteger>> firstsecondmnc = MathKit.GraphLinearEquationMNCBigInt(firstx, firsty, secondx, secondy);
					List<List<BigInteger>> firstthirdmnc = MathKit.GraphLinearEquationMNCBigInt(firstx, firsty, thirdx, thirdy);
					List<List<BigInteger>> secondthirdmnc = MathKit.GraphLinearEquationMNCBigInt(secondx, secondy, thirdx, thirdy);
